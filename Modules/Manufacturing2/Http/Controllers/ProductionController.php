...
class ProductionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $prod_id = Variation::where('id', $request->variation_id)->first()->product_id;
        $prod_name = Product::findOrFail($prod_id)->name;
        $search_products = Product::where('name', 'LIKE', "%$prod_name%")->get();
        dd($search_products);
        ...
        try {
            ...
            if ($production_sell->status == 'final') {
                ...
                // Create Products from the packaging.
                if($request->total_to_pack > 0){
                    $recipe->packaged_quantity = $request->total_to_pack;
                    $recipe->unpackaged_quantity = ($recipe->total_quantity - $request->total_to_pack);
                }
                if($request->tenkg_packaging_quantity > 0){
                    $recipe->ten_kg_bags = $request->tenkg_packaging_quantity;
                    $recipe->ten_kg_total_kgs = $request->total_10_kgs;
                    $this->productUtil->updateProductQuantity($request->location_id);
                }
                if($request->twentykg_packaging_quantity > 0){
                    $recipe->twenty_kg_bags = $request->twentykg_packaging_quantity;
                    $recipe->twenty_kg_total_kgs = $request->total_20_kgs;
                    $this->productUtil->updateProductQuantity($request->location_id);
                }
                if($request->fiftykg_packaging_quantity > 0){
                    $recipe->fifty_kg_bags = $request->fiftykg_packaging_quantity;
                    $recipe->fifty_kg_total_kgs = $request->total_50_kgs;
                    $this->productUtil->updateProductQuantity($request->location_id);
                }
                if($request->seventykg_packaging_quantity > 0){
                    $recipe->seventy_kg_bags = $request->seventykg_packaging_quantity;
                    $recipe->seventy_kg_total_kgs = $request->total_70_kgs;
                    $this->productUtil->updateProductQuantity($request->location_id);
                }
            }
