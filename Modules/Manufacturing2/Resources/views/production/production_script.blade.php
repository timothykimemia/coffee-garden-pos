<script type="text/javascript">
	...
	$(document).on('change', '#production_form #variation_id, #production_form #location_id #wasted_units', function () {
		var variation_id = $("#variation_id").val();
		var location_id = $("#location_id").val();
        
        var manufactured_quantity = __read_number($('#recipe_quantity')) - __read_number($('#wasted_units'));
	    __write_number($('#manufactured_quantity'), manufactured_quantity);
		...
		if(variation_id && location_id) {
			$.ajax({
	            ...
	            success: function(result) {
	                ...
	                __write_number($('#manufactured_quantity'), __read_number($('#recipe_quantity'))-__read_number($('#wasted_units')));
	                
                    var manufactured_quantity = document.getElementById("recipe_quantity").value;
                    
	                var initial_unpacked = result.recipe.unpackaged_quantity;
    	            __write_number($('#previously_unpacked_quantity'), initial_unpacked);
                	var total_unpacked = parseFloat(manufactured_quantity) + parseFloat(initial_unpacked);
                	__write_number($('#unpacked_quantity'), total_unpacked);
	...
	$(document).on('change', '#recipe_quantity', function () {
	    var manufactured_quantity = document.getElementById("recipe_quantity").value;
	    
	    __write_number($('#manufactured_quantity'), (parseFloat(manufactured_quantity) - parseFloat(__read_number($('#wasted_units')))));
		
        var initial_unpacked = document.getElementById("previously_unpacked_quantity").value;
    ...
    $(document).on('change', '#ten_quantity, #twenty_quantity, #fifty_quantity, #seventy_quantity', function() {
        
        var variation_id = $("#variation_id").val();
        var location_id = $("#location_id").val();
        // var initial_unpacked = 0;
		...
		if(variation_id && location_id) {
			$.ajax({
	            ...
	            success: function(result) {
                    ...
                    var ten_quantity = __read_number($('#ten_quantity'));
                    var twenty_quantity = __read_number($('#twenty_quantity'));
                    var fifty_quantity = __read_number($('#fifty_quantity'));
                    var seventy_quantity = __read_number($('#seventy_quantity'));
                    
                    var ten_kg_total = ten_quantity * 10;
                    var twenty_kg_total = twenty_quantity * 20;
                    var fifty_kg_total = fifty_quantity * 50;
                    var seventy_kg_total = seventy_quantity * 70;
        
                    var remaining_unpacked = total_unpacked - (ten_kg_total + twenty_kg_total + fifty_kg_total + seventy_kg_total);
                    var to_pack = ten_kg_total + twenty_kg_total + fifty_kg_total + seventy_kg_total
                    
                    if(remaining_unpacked < to_pack){
                        alert('Total number of kilograms is more than the remaining unpacked.');
                    }
                    
                    // __write_number($('#unpacked_quantity'), remaining_unpacked);
                    __write_number($('#total_10_kgs'), ten_kg_total);
                    __write_number($('#total_20_kgs'), twenty_kg_total);
                    __write_number($('#total_50_kgs'), fifty_kg_total);
                    __write_number($('#total_70_kgs'), seventy_kg_total);
                    __write_number($('#total_to_pack'), to_pack);
	            },
	        });
		}
